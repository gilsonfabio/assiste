...
const SelectEspecializacao = ({options, setSubTipo, texto }) => {
    ...
    function renderOption(item:tiposProps) {
        return (
            <TouchableOpacity className='' onPress={() => {
                setSubTipo(item.stdId);
                setTxt(item.stdDescricao);
                setModalVisible(false);
            }}>
                <Text className='text-[12px]'>{item.stdDescricao}</Text>
            </TouchableOpacity>
        )
    }
            ...
            >
                ...
                <Modal                     
                    ...
                    <View className='flex-row bg-gray-300 border-gray-600 h-auto w-80 items-center justify-between p-1'>
                        <FlatList 
                            data={options}
                            keyExtractor={(item) => String(item.stdId)} 
